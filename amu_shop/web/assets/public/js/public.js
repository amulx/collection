		...
		$(".email").blur(function(){
			if($(this).val()==""||$(this).val()==null){
				if(!$(this).siblings().hasClass("msgError")){
					...
					$(this).after("<h6 class='msgError'>请输入真实有效的邮箱或用户名！</h6>");
	...
	function subMyLogin(){
			...
			if(email==null||email==""){
				if(!$(".email").siblings().hasClass("msgError")){
					...
					$(".email").after("<h6 class='msgError'>请输入真实有效的用户名或邮箱</h6>");
                    var flag = "no";
				}
			}
            ...
            if(flag=='yes'){
//                alert('public.js->URL= '+user_login);
                var type = '';
                //验证邮箱的正则表达式
...
//                alert(type);return false;
                ...
                $.ajax({
                    type: "POST",
                    url: user_login,
                    data: { emailOrUsername : email,  password : password, type:type },
                    success: function(result){
    //                        alert(result.msg);
                        if(result.code == 200){
                            alert("登录成功"+result.code);
                            var vendorStatus = result.vendorStatus;
                            plateClose('login');
                            //这里根据具体情况跳到"用户首页"或"服务商首页"
                            if(vendorStatus<=2){
                                location.href = user_index;
                            }else{
                                location.href = user_index_vendor;
                            }
...
//                            alert(result.msg);
                            if(!$(".email").siblings().hasClass("msgError")){
                                $(".email").css({"border-color":"#F3696E"});
                                $(".email").after("<h6 class='msgError'>"+result.msg+"</h6>");
                                $(".email").parent("li").attr("class","none");
                            }
                        }
                    }
                });
...
$(function(){
    $("#orderList_tag ul:last").css("border-bottom","1px solid #e2e3e3");

})


$(function(){
    goTopEx("#home_nav_bar");				/* 回顶部...... */
})
/* 回顶部 ......*/
function goTopEx(toTopDiv){
    /* 回顶部 */
    var $toTopDiv = $(toTopDiv);
    if($toTopDiv.length == 0)return;

    var obj=document.getElementById("eachTop");

    var _previousScrollTop = 0;
    window.onscroll = function(){
        //显示隐藏回顶部
        $(document).scrollTop() > 0 ? obj.style.display="block" : obj.style.display="none";


        var scrollTop = $(window).scrollTop(),
            scrollDelta = scrollTop - _previousScrollTop;

        //alert(scrollTop+"---"+_previousScrollTop);
        if (scrollDelta <= 0) {

            //$(".store-top > div").css("position","fixed").fadeIn();
        }
        else if (scrollDelta > 0) {
            //$(".store-top > div").fadeOut(500,function(){$(this).css("position","absolute")});
        }
    ...
    document.onkeydown=function(obj){
        e = obj ? obj :(window.event ? window.event : null);
        if(e.keyCode==36){
            //$("html,body").animate({"scrollTop":0+"px"},300);
            //$(".store-top > div").css("position","fixed").fadeIn();
        }
